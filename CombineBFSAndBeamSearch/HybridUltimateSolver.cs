...

{
    ...
    // [CLASS SOLVER CHÍNH - CẬP NHẬT LOGIC 4 GÓC]
...

    {
...

        {
            ...
            // [PHASE 1] SHELL SOLVER (4 CORNER BUFFER STRATEGY)
...

        {
            ...
            // [GÓC TRÊN-PHẢI]: Dùng Hàng dọc bên cạnh làm buffer

            SolveCornerWithBuffer(

                p1Target: Idx(min, max - 1),

                p2Target: Idx(min, max),

                p1Setup: Idx(min, max - 1),

                p2Setup: Idx(min + 1, max - 1),

                rotateOp: new FastMove(max - 1, min, 2),

                coreSize: targetCoreSize,
            ...
            // [GÓC DƯỚI-PHẢI]: Dùng Hàng ngang bên trên làm buffer

            SolveCornerWithBuffer(

                p1Target: Idx(max - 1, max),

                p2Target: Idx(max, max),

                p1Setup: Idx(max - 1, max),

                p2Setup: Idx(max - 1, max - 1),
            ...
            // 3. BOTTOM EDGE (Ngang) - Đi từ Phải sang Trái

            var bottomPairs = new List<(int, int)>();

            for (int c = max; c > min + 1; c -= 2) bottomPairs.Add((Idx(max, c), Idx(max, c - 1)));

            ProcessSegment(bottomPairs, targetCoreSize, moves);



            // [GÓC DƯỚI-TRÁI]: ĐÂY LÀ CHỖ BẠN YÊU CẦU

            // Target: (max, min+1) và (max, min) -> Đỏ

            // Setup: (max-1, min+1) và (max, min+1) -> Xanh (dọc)

            // Rotate: (min, max-1, 2)

            SolveCornerWithBuffer(

                p1Target: Idx(max, min + 1),

                p2Target: Idx(max, min),

                p1Setup: Idx(max, min + 1),      // Setup 1 (dưới)

                p2Setup: Idx(max - 1, min + 1),  // Setup 2 (trên) -> Buffer dọc
            ...
            // 4. LEFT EDGE (Dọc) - Đi từ Dưới lên Trên

            var leftPairs = new List<(int, int)>();

            // Bắt đầu từ max-1 vì góc (max, min) đã xong

            for (int r = max - 1; r > min + 1; r -= 2) leftPairs.Add((Idx(r, min), Idx(r - 1, min)));

            ProcessSegment(leftPairs, targetCoreSize, moves);



            // [GÓC TRÊN-TRÁI]: Khép vòng

            // Target: (min+1, min) và (min, min)

            // Setup: (min+1, min+1) và (min+1, min) -> Ngang

            // Rotate: (min, min, 2)

            SolveCornerWithBuffer(

                p1Target: Idx(min + 1, min),

                p2Target: Idx(min, min),

                p1Setup: Idx(min + 1, min),

                p2Setup: Idx(min + 1, min + 1), // Buffer ngang
...

        {
            ...
            // Setup: Cố gắng đưa 1 cặp số giống nhau về vị trí setup
